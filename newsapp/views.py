...
from .forms import ArticleForm, NewsletterForm, SignUpForm
...
# Base directory of the Django project (one level above this file)
_BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


def _serve_file(filename: str, download_name: str):
    """Return a ``FileResponse`` that prompts a browser download."""
...
def download_planning(request):
    """Serve PLANNING.md as a direct browser download."""
    return _serve_file("PLANNING.md", "PLANNING.md")


def download_readme(request):
    """Serve README.md as a direct browser download."""
    return _serve_file("README.md", "README.md")
...
def newsletter_create(request):
    """
    Render and process the newsletter creation form.

    GET  /newsletters/new/  -- display a blank form
    POST /newsletters/new/  -- validate and create a newsletter

    Access: journalists and editors only.  On success the user is
    redirected to the newsletter listing with a confirmation message.
    """
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            newsletter.articles.set(form.cleaned_data.get("articles", []))
            messages.success(request, f'Newsletter "{newsletter.title}" created.')
            return redirect("newsletter-list")
    else:
        form = NewsletterForm()
...
@login_required
def newsletter_edit(request, pk: int):
    """
    Render and process the newsletter edit form.

    GET  /newsletters/<pk>/edit/ -- display the pre-filled form
    POST /newsletters/<pk>/edit/ -- validate and save changes

    Access: the newsletter's own journalist or any editor.
    """
    newsletter = get_object_or_404(Newsletter, pk=pk)
    if not (request.user.is_editor or newsletter.author == request.user):
    ...
    if request.method == "POST":
        form = NewsletterForm(request.POST)
        if form.is_valid():
            newsletter.title = form.cleaned_data["title"]
            newsletter.description = form.cleaned_data["description"]
            newsletter.articles.set(form.cleaned_data.get("articles", []))
            newsletter.save()
    ...
    return render(
        request,
        "newsapp/newsletter_form.html",
        {"form": form, "action": "Edit", "newsletter": newsletter},
    )


@login_required
@require_POST
def newsletter_delete(request, pk: int):
    """
    Delete a newsletter via an HTML form POST.

    POST /newsletters/<pk>/delete/

    Access: the newsletter's own journalist or any editor.  After
    deletion the user is redirected to the newsletter listing.
    """
    newsletter = get_object_or_404(Newsletter, pk=pk)
    if not (request.user.is_editor or newsletter.author == request.user):
